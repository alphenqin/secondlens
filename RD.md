...
本文档根据 `doc/供应商受控外联情报二次研判标准/供应商受控外联情报二次研判标准.docx` 整理，用于指导供应商侧实现受控外联情报二次研判任务的接收、处理与结果回传。
...
2. 任务获取：供应商监控只读桶，发现新任务后下载待研判情报文件。
...
拒绝文件由网防云侧生成。网防云侧会将供应商提交的原结果文件复制到只读桶，并在原结果基础上追加审核拒绝字段：
...
供应商主流程只需要读取待研判任务，并将结果写入只写桶；关联告警与拒绝结果文件当前不进入自动处理流程。拒绝结果文件主要用于网防云侧审核反馈，当前供应商侧不实现基于拒绝原因的自动整改和重提逻辑。
